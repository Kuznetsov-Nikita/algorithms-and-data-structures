...
struct node {
    long long int left, right, parent, link;
    std::map<char, long long int> to;

    node(long long int left = 0, long long int right = 0, long long int parent = -1): left(left), right(right), parent(parent), link(-1) {}
...
void find_k_string(const std::vector<node>& suffix_tree, long long int n, long long int v, long long int k, long long int& cur_num, std::pair<long long int, long long int>& answer) {
    if (suffix_tree[v].right == n) {
        cur_num += suffix_tree[v].right - suffix_tree[v].left - 1;
    } else {
        cur_num += suffix_tree[v].right - suffix_tree[v].left;
    }

    if (cur_num >= k) {
        answer = std::make_pair(v, cur_num - k);
        return;
    } else {
        for (auto it = suffix_tree[v].to.begin(); it != suffix_tree[v].to.end(); ++it) {
            find_k_string(suffix_tree, n, (*it).second, k, cur_num, answer);
...
std::string get_ans(const std::vector<node>& suffix_tree, std::string& s, long long int v, long long int n) {
    std::string ans;
    if (suffix_tree[v].right == s.size() + 1) {
        ans = s.substr(suffix_tree[v].left, suffix_tree[v].right - suffix_tree[v].left - n - 1);
    } else {
        ans = s.substr(suffix_tree[v].left, suffix_tree[v].right - suffix_tree[v].left - n);
    }
...
int main() {
    ...
    find_k_string(suffix_tree, s.size() + 1, 0, k, cur_num, ans);

    if (ans == std::pair<long long int, long long int> (0, 0)) {
        long long int new_cur_num = 0;
        find_k_string(suffix_tree, s.size() + 1, 0, cur_num, new_cur_num, ans);
    }
